/*
 * All-pairs bozorth3 score matrix for a set of P5 PGM captures, but with an
 * investigation-only relaxed Bozorth floor so captures with <10 minutiae still
 * produce raw scores.
 *
 * Loads each PGM, runs NBIS minutiae detection, then prints an NxN matrix of
 * raw bozorth3 scores.
 *
 * Usage: eh577-pgm-match-relaxed <a.pgm> [b.pgm ...] [--threshold N]
 *
 * Copyright (C) 2026 workspace contributors
 * LGPL-2.1-or-later
 */

#define FP_COMPONENT "eh577-pgm-match-relaxed"
...
extern int  eh577_relaxed_bozorth_probe_init (struct xyt_struct *);
extern int  eh577_relaxed_bozorth_to_gallery (int, struct xyt_struct *, struct xyt_struct *);
extern int  EH577_RELAXED_MIN_COMPUTABLE_BOZORTH_MINUTIAE (void);
...
print_matrix (Ctx *c)
{
  ...
  printf ("\nRelaxed Bozorth3 score matrix (%d images, threshold=%d, min-computable-minutiae=%d)\n\n",
          c->n,
          c->threshold,
          EH577_RELAXED_MIN_COMPUTABLE_BOZORTH_MINUTIAE ());
  printf ("     ");
  for (i = 0; i < c->n; i++)
    printf (" %4d", i + 1);
  printf ("\n");
  ...
  for (i = 0; i < c->n; i++)
    {
      int probe_len;
      printf ("%3d |", i + 1);
      probe_len = eh577_relaxed_bozorth_probe_init (&c->xyt[i]);
      for (j = 0; j < c->n; j++)
        {
          int score = eh577_relaxed_bozorth_to_gallery (probe_len, &c->xyt[i], &c->xyt[j]);
  ...
  if (file_argc < 2)
    {
      ...
      printf ("  Prints an NxN relaxed bozorth3 score matrix for all pairs.\n");
